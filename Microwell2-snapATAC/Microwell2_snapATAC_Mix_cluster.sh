...
correctBC_script=/share/home/guoguoji/JiaqiLi/tools/MWATAC/Microwellseq2_correctBC_mismatch.py
...
${dropseq_root}/TagBamWithReadSequenceExtended SUMMARY=${outdir}/unaligned_tagged_Cellular.bam_summary_R2.txt \
    BASE_RANGE=1-6:22-27:43-48:84-93 BASE_QUALITY=10 BARCODED_READ=1 TAG_BARCODED_READ=false DISCARD_READ=false TAG_NAME=CB NUM_BASES_BELOW_QUALITY=1 \
...
# FilterBAM
${dropseq_root}/FilterBAM I=$outdir/Aligned.out.bam O=$outdir/mouse.bam REF_SOFT_MATCHED_RETAINED=MOUSE
${dropseq_root}/FilterBAM I=$outdir/Aligned.out.bam O=$outdir/human.bam REF_SOFT_MATCHED_RETAINED=HUMAN
...
snaptools snap-pre \
    --input-file=$outdir/mouse.bam \
	--output-snap=$outdir/mouse.snap \
	--genome-name=mm10 \
...
snaptools snap-pre \
    --input-file=$outdir/human.bam \
	--output-snap=$outdir/human.snap \
	--genome-name=hg19 \
	--genome-size=$outdir/mix.chrom.sizes \
	--min-mapq=30 \
	--min-flen=0 \
...
snaptools snap-add-bmat \
    --snap-file=$outdir/mouse.snap \
    --bin-size-list 5000 10000 \
    --verbose=True

snaptools snap-add-bmat \
    --snap-file=$outdir/human.snap \
    --bin-size-list 5000 10000 \
    --verbose=True

# rm tmpdir if final output exist
if [ -f $outdir/human.snap -a -f $outdir/mouse.snap ]; then
    rm -r $tmpdir
fi
